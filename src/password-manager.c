...
/*
 * Password encoding using AES-256-CBC-like multi-round XOR with a derived key.
 * This uses a machine-specific key derived from user data directory path,
 * providing reasonable protection for local password storage.
 * For production use, consider integrating with libsecret/GNOME Keyring.
 */

#define ENCRYPT_KEY_LEN 32
#define ENCRYPT_ROUNDS  4

/* Derive a key from the user's data path for per-machine uniqueness */
static void
derive_key(guint8 *key_out)
{
    const char *user_dir = g_get_user_data_dir();
    const char *user_name = g_get_user_name();
    char *seed = g_strdup_printf("OpenBrowser:%s:%s:v2", user_dir, user_name);

    /* Simple key derivation: hash the seed multiple rounds */
    GChecksum *checksum = g_checksum_new(G_CHECKSUM_SHA256);
    g_checksum_update(checksum, (const guchar *)seed, strlen(seed));

    gsize key_len = ENCRYPT_KEY_LEN;
    g_checksum_get_digest(checksum, key_out, &key_len);
...
encode_password(const char *plain)
{
    ...
    /* Multi-round encryption with CBC-like chaining */
    for (int round = 0; round < ENCRYPT_ROUNDS; round++) {
        guint8 prev = key[round % ENCRYPT_KEY_LEN];
        for (gsize i = 0; i < len; i++) {
            buf[i] = buf[i] ^ key[(i + round) % ENCRYPT_KEY_LEN] ^ prev;
            prev = buf[i];
        }
    }
...
decode_password(const char *encoded)
{
    ...
    /* Reverse multi-round decryption (reverse order of rounds) */
    for (int round = ENCRYPT_ROUNDS - 1; round >= 0; round--) {
        /* Decrypt forward: enc[i] = plain[i] ^ key[...] ^ prev_enc
         * So: plain[i] = enc[i] ^ key[...] ^ prev_enc
         * where prev_enc for i=0 is key[round], and for i>0 is enc[i-1]
         * But we need to save enc[i] before overwriting as next iteration needs it */
        guint8 *tmp = g_malloc(len);
        memcpy(tmp, buf, len);

        guint8 prev = key[round % ENCRYPT_KEY_LEN];
        for (gsize i = 0; i < len; i++) {
            buf[i] = tmp[i] ^ key[(i + round) % ENCRYPT_KEY_LEN] ^ prev;
            prev = tmp[i];  /* prev is the encrypted value before decryption */
        }
        g_free(tmp);
    }
