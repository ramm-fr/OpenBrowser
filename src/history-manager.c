...
struct _HistoryManager {
    ...
    GList *entries;
    char *data_file;
    guint save_timeout_id;  /* Debounced save timer */
};
...
/* Debounced save: coalesces rapid saves into one disk write after 5 seconds idle */
static gboolean
history_save_timeout_cb(gpointer user_data)
{
...
static void
history_manager_schedule_save(HistoryManager *self)
{
    ...
    self->save_timeout_id = g_timeout_add_seconds(5, history_save_timeout_cb, self);
}
...
history_manager_dispose(GObject *object)
{
    ...
    /* Cancel pending save timer and save immediately on shutdown */
    if (self->save_timeout_id > 0) {
        g_source_remove(self->save_timeout_id);
        self->save_timeout_id = 0;
    }
...
history_manager_add(HistoryManager *self, const char *title, const char *uri)
{
    ...
    for (GList *l = self->entries; l; l = l->next) {
        ...
        if (g_strcmp0(entry->uri, uri) == 0) {
            ...
            /* Defer save to avoid blocking the UI on every navigation */
            history_manager_schedule_save(self);
            return;
        }
    }
...
on_history_clear_clicked(GtkButton *button, gpointer user_data)
{
    ...
    /* Update UI: hide the list, show empty state */
    GtkWidget *page = g_object_get_data(G_OBJECT(button), "page-widget");
    GtkWidget *list_widget = g_object_get_data(G_OBJECT(button), "list-widget");
    GtkWidget *count_widget = g_object_get_data(G_OBJECT(button), "count-widget");

    if (list_widget) {
        gtk_widget_set_visible(list_widget, FALSE);
    }
    if (count_widget) {
        gtk_widget_set_visible(count_widget, FALSE);
    }
    ...
    if (page) {
        ...
        GtkWidget *empty_title = gtk_label_new("History cleared");
...
static void
on_history_item_delete_clicked(GtkButton *button, gpointer user_data)
{
    HistoryManager *self = HISTORY_MANAGER(user_data);
    ...
    if (uri && *uri) {
        history_manager_remove(self, uri);
        if (row) {
...
history_manager_create_full_page(HistoryManager *self)
{
    ...
    if (!all_entries) {
        ...
        for (GList *l = all_entries; l && count < 30; l = l->next, count++) {
            ...
            g_object_set_data_full(G_OBJECT(del_btn), "uri", g_strdup(entry->uri), g_free);
            g_object_set_data(G_OBJECT(del_btn), "row-widget", row);
            g_signal_connect(del_btn, "clicked", G_CALLBACK(on_history_item_delete_clicked), self);
        ...
        /* Store widget references on clear button for UI update */
        g_object_set_data(G_OBJECT(clear_btn), "page-widget", page);
        g_object_set_data(G_OBJECT(clear_btn), "list-widget", list_box);
        g_object_set_data(G_OBJECT(clear_btn), "count-widget", count_label);
    }
