...
on_download_finished(WebKitDownload *download, gpointer user_data)
{
    (void)download;
    DownloadItem *item = (DownloadItem *)user_data;
    /* Don't mark as completed if it was cancelled (failed fires before finished) */
    if (!item->cancelled) {
        item->completed = TRUE;
        item->progress = 1.0;
    }
...
download_manager_add_download(DownloadManager *self, WebKitDownload *download)
{
    ...
    if (self->live_list_box && gtk_widget_get_parent(self->live_list_box)) {
        /* Hide empty state if visible, show list */
        if (self->empty_box) {
            gtk_widget_set_visible(self->empty_box, FALSE);
        }
        gtk_widget_set_visible(self->live_list_box, TRUE);
...
on_live_download_done(WebKitDownload *download, gpointer user_data)
{
    ...
    if (ADW_IS_ACTION_ROW(row)) {
        /* Check if the download was cancelled (failed fires before finished) */
        DownloadManager *dm = download_manager_get_default();
        gboolean was_cancelled = FALSE;
        for (guint i = 0; i < dm->downloads->len; i++) {
            DownloadItem *item = g_ptr_array_index(dm->downloads, i);
            if (item->download == download) {
                was_cancelled = item->cancelled;
                break;
            }
        }
        if (was_cancelled) {
...
download_manager_create_full_page(DownloadManager *self)
{
    ...
    /* Empty state - shown/hidden based on downloads count */
    ...
    /* Always add list_box to page so live updates work */
    gtk_box_append(GTK_BOX(page), list_box);

    if (self->downloads->len == 0) {
        /* Show empty, hide list */
        gtk_widget_set_visible(empty_box, TRUE);
        gtk_widget_set_visible(list_box, FALSE);
    } else {
        /* Show list, hide empty */
        gtk_widget_set_visible(empty_box, FALSE);
